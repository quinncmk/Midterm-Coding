...
public abstract class Person {
	...
	// To print phone number, if the format isn't correct, it will throw an exception
	public void PrintPhoneNumber() throws PersonException{
		// If phone number is correct then format it to (123)-456-7890
		if (!this.phone_number.matches("\\(\\d{3}\\)-\\d{3}-\\d{4}")) {
	...
	public int PrintAge() throws PersonException{
		...
		// If age is greater than 100, it will throw an exception
		if (age > 100) {
			throw new PersonException(null);
		}
